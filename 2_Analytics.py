...
with tab3:
    ...
    if runs.empty:
        ...
        import numpy as np
        dev = next((d for d in core.get_devices_cached() if d.name == name), None)
        power_kw = float(getattr(dev, "power_kw", 0.0) or 0.0)
        ss = core.load_studio_settings()
        rate = float(ss.get("rate_per_kwh") or 0.0)
        currency = ss.get("currency", "USD")

        if power_kw <= 0 or rate <= 0:
            missing = []
            if power_kw <= 0:
                missing.append(f"**{name}**'s rated kW")
            if rate <= 0:
                missing.append("the electricity rate")
            st.info("To see energy (kWh) and cost per firing, set "
                    + " and ".join(missing)
                    + " under **Settings → Energy & open data**. "
                    "Energy is estimated from output% × rated kW.")
        ...
        def _group_kwh(g):
            ...
            out = g["output_pct"].astype(float).fillna(0).to_numpy() / 100.0
            if len(out) < 2:
                return 0.0
            # total_seconds() is correct regardless of datetime resolution
            dt = g["timestamp_utc"].diff().dt.total_seconds().to_numpy()[1:]
            med = np.median(dt) if len(dt) else 300.0
            cap = max(3 * med, 600.0)
            frac = (out[1:] + out[:-1]) / 2.0
            mask = (dt > 0) & (dt <= cap)
            return round(float((power_kw * frac[mask] * dt[mask] / 3600.0).sum()), 2)
        ...
        energies = runs.groupby("run_id")[["timestamp_utc", "output_pct"]].apply(
            _group_kwh)
        firings["energy_kwh"] = list(energies.values)
        if rate > 0 and power_kw > 0:
            firings["cost"] = firings["energy_kwh"].apply(
                lambda k: round(k * rate, 2) if k is not None else None)

        st.dataframe(firings, use_container_width=True, hide_index=True)

        if power_kw > 0:
            total_kwh = float(firings["energy_kwh"].fillna(0).sum())
            line = f"Window total: **{total_kwh:,.1f} kWh** across {len(firings)} firing(s)"
            if rate > 0:
                line += f" · est. **{total_kwh*rate:,.2f} {currency}**"
            st.caption(line + f"  (at {power_kw:g} kW rated"
                       + (f", {rate:g} {currency}/kWh)" if rate > 0 else ")"))
